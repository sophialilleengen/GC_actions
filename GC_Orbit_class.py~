...
class GCorbit:
	def __init__(self,r_pc=None,rho_Msunpc3=None,bhmass_Msun=None,inputfilename=None,n=None): 
		...
		pot_y=self.potential(r,density,low=np.min(r),high=np.max(r),x_i=None,w_i=None,full_integration=True) #   #11. What units??? Write it in code as comment.
		self._G=un.m**3/(un.kg*un.s**2).to.(un.pc**3/(un.solMass*un.s**2), cs.G) #[pc³/M_sun*s²]
		self._bhmass=bhmass_Msun	#M_sun
		self._n=80
		self._x_i=np.polynomial.legendre.leggauss(self.n)[0]        #    4. Maybe you want to calculate the xi and wi already in __init__ as well and pass them to the potential function...
		self._w_i=np.polynomial.legendre.leggauss(self.n)[1]


		self.pot=interpolate.InterpolatedUnivariateSpline(self.r,pot_y) #^-- 5. Did you test if it makes sense to interpolate the potential in log-space as well?
	...
	def _r_derivative(self):   #17. Sorry, no idea, what you're doing in this function.
	"""
	NAME:
	...
	def orbit(self,x0,y0,z0,vx0,vy0,vz0,dt=None,t_end=None):     #22. Is this the orbit integration function? I would, in addition to the initial star position, have delta_t and t_end (in useful units) as parameters. N is then calculated in the function from that.
	"""
	NAME:
